...
function connHandler(conn) {
  ...
  conn.on('data', function(chunk) {
    ...
    try {
      ...
      conn.write(JSON.stringify(generateError('Invalid data from client.')));
      conn.end();
      return;
    }
    ...
    linter(lintOpts)
      .then(function(data) {
        ...
        if (formatter === 'string') {
          ...
          for (var i in outputs) {
            ...
            for(var j in invalidOptionWarnings) {
              var iow = invalidOptionWarnings[j];
      ...
      .catch(function(error) {
        ...
        var errorObject = generateError(error);
